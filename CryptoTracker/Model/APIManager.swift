...
class APIManager{
    ...
    func getCurrentCryptoRates(completion: @escaping (CryptoRate?, Error?) -> Void) {
        Alamofire.request(APIRouter.getCurrentCryptoRates)
            .responseJSON { response in
                ...
                case 200 ... 299: // Success
                    guard let result = try? JSONDecoder().decode(CryptoRate.self,
                                                                 from: response.data!)
                    else {
    ...
    fileprivate func handleError(_ response: DataResponse<Any>,
        ...
        do {
            let errorResult = try JSONDecoder().decode(String.self, from: response.data!)
